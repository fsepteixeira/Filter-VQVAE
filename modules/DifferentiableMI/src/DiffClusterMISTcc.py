...
class DiffClusterMISTcc(nn.Module):
    """
    ...
    def __init__(self, parameters):
        super(DiffClusterMISTcc, self).__init__()
        ...
        elif parameters["dist_metric_x"] == "mae_dist":
            self.distance_x = PairwiseMAEDists(parameters["range_"])
    ...
    def _mutual_information(self, x, y):
        ...
        nx_i = torch.sum(gtz_x, dim=1) - 2
        ny_i = torch.sum(gtz_y, dim=1) - 2
        
        # Compute digamma of count
        nx_i_digamma  = torch.digamma(nx_i.float()+1e-1)
        ny_i_digamma  = torch.digamma(ny_i.float()+1e-1)
        ...
        mutual_information = mutual_information - n_avg_digamma
